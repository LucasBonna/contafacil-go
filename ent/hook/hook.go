...
func (f EmissionFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.EmissionMutation); ok {
		return f(ctx, mv)
	}
...
// The GnreEmissionFunc type is an adapter to allow the use of ordinary
// function as GnreEmission mutator.
type GnreEmissionFunc func(context.Context, *ent.GnreEmissionMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f GnreEmissionFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.GnreEmissionMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.GnreEmissionMutation", m)
}
